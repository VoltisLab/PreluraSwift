...
class InboxViewModel: ObservableObject {
    ...
    /// Sum of unread badges across inbox threads (Messages tab); chat alerts stay off the bell list until stale.
    ...
    func loadConversationsAsync(preview: (id: String, text: String, date: Date)?, currentUsername: String? = nil) async {
        ...
        do {
            ...
            // When the list resolver lags (e.g. sale row in chat but `lastMessage` still offer-shaped), keep the row we already showed.
            ...
            if let preview = preview, let idx = list.firstIndex(where: { $0.id == preview.id }) {
                ...
                // Prefer local preview when it is newer or within a short window (clock / ordering skew vs API `lastMessageTime`).
    ...
    /// Keep list subtitle when API still returns an offer-shaped line but we already showed sale/order progress (resolver lag).
